                    ...
                    when (signalingState) {
                        ...
                        SignalingState.ANSWERED -> {
                            ...
                            if (mMediaState == MediaState.CONNECTED) {
                                ...
                                //start the time
                                binding.vTimer.visibility= VISIBLE
                                ...
                                handler.postDelayed(Runnable {
                                    ...
                                    val saat = TimeUnit.MILLISECONDS.toHours(runTIme) % 24
                                    val dakika = TimeUnit.MILLISECONDS.toMinutes( runTIme) % 60
                                    val saniye = TimeUnit.MILLISECONDS.toSeconds(runTIme) % 60
                                    binding.tvTimer.text = ("${timeToString(saat)}:${timeToString(dakika)}:${timeToString(saniye)}")
